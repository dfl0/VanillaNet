...
class progressive_activation(nn.ReLU):
    def __init__(self, dim, act_num=3, deploy=False, groups=1):
        super(progressive_activation, self).__init__()
        self.act_num = act_num
        self.deploy = deploy
        self.dim = dim
        assert groups >= 1 and dim % groups == 0
        self.groups = groups
        self.weight = torch.nn.Parameter(torch.randn(dim, dim // groups, act_num*2 + 1, act_num*2 + 1))
        if deploy:
            self.bias = torch.nn.Parameter(torch.zeros(dim))
        else:
            self.bias = None
    ...
    def forward(self, x):
        if self.deploy:
            return torch.nn.functional.conv2d(
                super(progressive_activation, self).forward(x),
                self.weight, self.bias, padding=self.act_num, groups=self.groups)
        else:
            return self.bn(torch.nn.functional.conv2d(
                super(progressive_activation, self).forward(x),
                self.weight, padding=self.act_num, groups=self.groups))
...
class EfficientBlock(nn.Module):
    def __init__(self, dim, dim_out, act_num=3, stride=2, deploy=False, groups=1, stage_idx=0):
        super().__init__()
        self.act_learn = 1
        self.deploy = deploy
        self.act_learn = 1
        assert dim_out % groups == 0 and dim % groups == 0
        self.groups = groups
        progressive_act_num = act_num + stage_idx
        if self.deploy:
            self.conv = nn.Conv2d(dim, dim_out, kernel_size=1)
        else:
            self.conv1 = nn.Sequential(
        ...
        self.pool = nn.Identity() if stride == 1 else nn.MaxPool2d(stride)
        self.act = progressive_activation(dim_out, progressive_act_num, deploy=self.deploy, groups=self.groups)
...
class VanillaNet(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, dims=[96, 192, 384, 768], 
        ...
        group_config = [16, 32, 64, 128]
        self.stages = nn.ModuleList()
        for i in range(len(strides)):
            if not ada_pool:
                # stage = Block(dim=dims[i], dim_out=dims[i+1], act_num=act_num, stride=strides[i], deploy=deploy)
                stage = EfficientBlock(dim=dims[i], dim_out=dims[i+1], act_num=act_num, stride=strides[i], deploy=deploy, groups=group_config[i], stage_idx=i)
            else:
